function [rho, stats] = MLmaxent_apg(operators, f, lbd, opts)

% **Modified QSE_APG code for minimizing entropy under the ML constraints.
...
defaults.threshold_fval = -Inf; % stop when negative log likeilhood less than this
...
defaults.bb = false; % use Barzilai-Bowein to initialize step sizes instead of afactor increases
defaults.accel = 'fista_tfocs'; % different ways to grow and compute beta

% tweak behavior parameters 
defaults.t0 = 0.9; %%% % set to a scalar if we know a good starting t, set to [] for Lipschitz estimation
...
for i=1:opts.imax
    ...
    if varrho_changed
        ...
        gradient_like = -qmt(coeff_temp, operators, 'adjoint');%%% no consequence 
        %%% since gradient is updated below with the entropy version.
                
        try
            [V,D] = eig(varrho);%%% logrho calculation
        catch ME
            warning(ME.message);
            NANerr=sum(sum(isnan(varrho)))
            [V,D] = eig(varrho,eye(d),'qz');  % For example
%             [V,D] = eig((varrho+1e-5.*eye(d))./trace(varrho+1e-5.*eye(d)));  % For example
        end
        Dtrunc = max(diag(D),0);%%%
        Dtrunc = (Dtrunc+1e-10)./sum(Dtrunc+1e-10);%%%
        logrho = V*diag(real(log(Dtrunc)))*V';%%%
                
        gradient_ent = (logrho+eye(d));%%% negative entropy gradient calculation
        gradient = mu.*gradient_like+lbd.*gradient_ent;%%%
        
        stats.comp_grad = stats.comp_grad+1;
        % fval_varrho = real(sum(sum(varrho.'.*logrho)));%%% negative entropy
        fval_varrho = -mu.*f'*log(probs_varrho)+lbd.*real(sum(sum(varrho.'.*logrho)));
        stats.comp_fval = stats.comp_fval+1;
    ...
    while ~t_good
        ...
        try
            [V,D] = eig(rho_new);%%% logrho calculation
        catch ME
            warning(ME.message);
            rho_new
            NANerr=sum(sum(isnan(rho_new)))
            [V,D] = eig(rho_new,eye(d),'qz');  % For example
%             [V,D] = eig((rho_new+1e-5.*eye(d))./trace(rho_new+1e-5.*eye(d)));  % For example
        end
        Dtrunc = max(diag(D),0);%%%
        Dtrunc = (Dtrunc+1e-10)./sum(Dtrunc+1e-10);%%%
        logrho_new = V*diag(real(log(Dtrunc)))*V';%%%
        trrlr_new = real(sum(Dtrunc.*log(Dtrunc+1e-15)));
        %fval_new = real(sum(eigrho_new.*log(eigrho_new+1e-15)));%%% new negative entropy
        fval_new = -mu.*f'*log(probs_rho_new)+lbd.*trrlr_new;
        stats.comp_prob = stats.comp_prob+1;
        stats.comp_fval = stats.comp_fval+1;
...
%         R=qmt(f./probs_rho_new, operators, 'adjoint');
%         test = norm((R-eye(d)-lbd.*(logrho_new-trrlr_new.*eye(d)))*rho_new)
%         stats.dists(i) = test;
...
%         R=qmt(f./probs_rho_new, operators, 'adjoint');
%         test = norm((R-eye(d)-lbd.*(logrho_new-trrlr_new.*eye(d)))*rho_new)
        %test = norm((R-lbd.*(logrho_new+eye(d))));
        break;
    end
